...
class URoomBookingService extends AbstractServices {
  ...
  public async createRoomBooking(req: Request) {
    return await this.db.transaction(async (trx) => {
      ...
      const checkUser = await clientModel.getSingleUser({
        email,
        hotel_id,
      });
      ...
      if (!checkUser.length) {
        // create user
        userRes = await clientModel.createUser({
          name,
          email,
          hotel_id,
          nid_no,
      ...
      const checkUnavailableRoom = allBookingRoom.filter(
      ...
      if (paid_amount > 0) {
        ...
        await invoiceModel.createMoneyReciept({
          hotel_id,
          created_by: user_id,
          user_id: userID,
          payment_type,
      ...
      if (paid_amount > 0) {
        ...
        if (due_amount) {
          ...
          await clientModel.updateSingleUser(
